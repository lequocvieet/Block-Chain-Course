...
func main() {
	...
	sig, prefixed := Sign(message, key)

	fmt.Println("address:", hex.EncodeToString(crypto.PubkeyToAddress(key.PublicKey).Bytes()))
	fmt.Println("signature:", hex.EncodeToString(sig))
	fmt.Println("prefixed", prefixed)
	fmt.Println("prefixed string", string(prefixed))
	fmt.Println("prefixed to hex", hex.EncodeToString(prefixed))
}
